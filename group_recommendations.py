...
def group_preds(userlist, n):
    """
    This function takes in a user list, calculates the improved top n
    recommendations the group.

    :param n: int, the number of recommendations we want
    ...
    group_recommendations = mean_rating(group_no_na, n, 'means')

    return group_recommendations


def get_group_no_na(userlist):
    """
    Gets the dataframe of all the movies with predicted ratings for the group
    in which all the users have predicted scores
    :param userlist: list, the users of interest
    :return: df, see above
    """
    group_df = group_dataframe(userlist, 20, 10000)
    group_no_na = group_df.dropna()
