...
def check_if_movie_available(movie_name, moviedata):
    ...
    print()
    movie_id = input(
        "Type the movie ID of the movie you are asking about or 'q' if it "
                ...
                if not in_similar:
                    for user in no_simil:
                        print(f"None of the users similar to user {user} have "
                              f"rated the movie, so we cannot recommend it to "
                ...
                # Checking if the score is tied with the last recommended item
                    ...
                    if tie:
                        ...
                        print(f"With a predicted rating of {rating:.2f}, this "
                              f"movie is ranked at position {position} of the "
                              f"list of possible recommendations, so it was "
                              f"not suggested in your top "
                              f"{number_of_recommendations} list.")
...
def main():
    ...
    if explanation_needed.lower() == "y":
        ...
        if type_of_issue == "1":
            ...
            genre_data = pd.read_csv("genre_info.csv")
